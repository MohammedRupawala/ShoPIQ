...
export const pieChart = TryCatch(async (req,res,next)=>{
    ...
    let charts
    if(myCache.has(cacheKey)) charts = JSON.parse(myCache.get(cacheKey) as string) 
        else{
           
            const[ shippingOrderCount,
                processOrderCount,
                deliveredOrderCount,
                allCategories,
                productCount,
            ...
            const grossIncome = completeRevenueStats.reduce(
                (gross,current)=>  gross + (current.total || 0),0
            )
            const discount = completeRevenueStats.reduce(
                (discount,current) => discount + (current.discount || 0),0
            )
